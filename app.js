...
//}
	if(cmd === `${prefix}lvl`){
		let lvlembed = new Discord.RichEmbed()
		        .setTitle(`Rangi za poziomy`)
		.setColor(3447003)
        .addField('`Poziom:` 1', '<@&536509855624855592>\n- 100 Jonków.', true)
        .addField('`Poziom:` 5', '<@&469087376845701136>\n- Przyznaje dostęp do konkursów premium.\n- Dostęp do kanału do zareklamowania się.\n- 300 Jonków.', true)
...
if(cmd === `${prefix}profli`){
  ...
  if (cmd === `${prefix}rangi`){
    ...
    message.channel.send("**Aby się zweryfikować, należy zaznaczyć reakcję na <#527242048764379182>.");
	 }
...
.setFooter(`${user.username}#${user.discriminator}`)
      ...
      ({timeout: 1000});
		...
		gchannel.send("Na <#434407891483492383> pojawiła się nowa propozycja.");
...
.setFooter(`${user.username}#${user.discriminator}`)
      ...
      ({timeout: 1000});
		...
		gchannel.send("Na <#495654275276472340> pojawiła się nowa propozycja.");
}     //Closes the if (command === 'suggest'){ 
...
//if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Nie masz uprawnień do używania tej k0z4k komendy.");
 ...
 // }
  ...
  if(cmd === `${prefix}avatar`) {
    ...
    // Create Embed
    let embedan = new Discord.RichEmbed()
...
let user = message.mentions.users.first() || message.author;
		 ...
		 if(gra === "w grze Custom Status") gra = "posiada Customowy Status przez co nie mogę wykryć w jaką grę gra";
...
let gej = message.mentions.users.first() || message.author;
    let gayembed = new Discord.RichEmbed()
        .setColor("#f442d4")
        .setTitle(`:gay_pride_flag: **${gej.username} jest w ${gay}% gejem!** :gay_pride_flag:`);
